...
def get_l2_penalty(nnet, pow=2):
    ...
    return T.cast(l2_penalty, dtype='float32')
...
def create_update(nnet):
    ...
    if Cfg.softmax_loss:
        ...
        compile_update_msvdd(nnet, inputs, targets)
...
def compile_update_default(nnet, inputs, targets):
    ...
    train_loss = T.cast((objective) / targets.shape[0], dtype='float32')
    train_acc = T.cast(train_acc * 1. / targets.shape[0], dtype='float32')
    ...
    test_loss = T.cast(objective / targets.shape[0], dtype='float32')
    test_acc = T.cast(test_acc * 1. / targets.shape[0], dtype='float32')
...
def compile_update_softmax(nnet, inputs, targets):
    ...
    if Cfg.ad_experiment:
        train_loss = T.mean(l_objectives.binary_crossentropy(prediction.flatten(), targets), dtype='float32')
        train_acc = T.mean(l_objectives.binary_accuracy(prediction.flatten(), targets), dtype='float32')
    else:
        train_loss = T.mean(l_objectives.categorical_crossentropy(prediction, targets), dtype='float32')
        train_acc = T.mean(T.eq(T.argmax(prediction, axis=1), targets), dtype='float32')
    ...
    if Cfg.ad_experiment:
        test_loss = T.mean(l_objectives.binary_crossentropy(test_prediction.flatten(), targets), dtype='float32')
        test_acc = T.mean(l_objectives.binary_accuracy(test_prediction.flatten(), targets), dtype='float32')
    else:
        test_loss = T.mean(l_objectives.categorical_crossentropy(test_prediction, targets), dtype='float32')
        test_acc = T.mean(T.eq(T.argmax(test_prediction, axis=1), targets), dtype='float32')
...
def compile_update_svdd(nnet, inputs, targets):
    ...
    dist = T.sum(((rep - nnet.cvar.dimshuffle('x', 0)) ** 2), axis=1, dtype='float32')
    scores = dist - nnet.Rvar
    stack = T.stack([T.zeros_like(scores), scores], axis=1)
    loss = T.cast(T.sum(T.max(stack, axis=1)) / (inputs.shape[0] * nu), dtype='float32')

    y_pred = T.argmax(stack, axis=1)
    acc = T.cast((T.sum(T.eq(y_pred.flatten(), targets), dtype='int32') * 1. / targets.shape[0]), 'float32')
    ...
    if Cfg.reconstruction_penalty:
        ...
        rec_loss = T.sum(rec_loss, axis=range(1, ndim), dtype='float32')
        rec_penalty = (1/C_rec) * T.mean(rec_loss)
    else:
        rec_penalty = T.cast(0, dtype='float32')
    ...
    avg_dist = T.mean(dist, dtype='float32')
    ...
    test_dist = T.sum(((test_rep - nnet.cvar.dimshuffle('x', 0)) ** 2), axis=1, dtype='float32')

    test_scores = test_dist - nnet.Rvar
    test_stack = T.stack([T.zeros_like(test_scores), test_scores], axis=1)
    test_loss = T.cast(T.sum(T.max(test_stack, axis=1)) / (inputs.shape[0]*nu), dtype='float32')
    ...
    if Cfg.reconstruction_penalty:
        ...
        test_rec_loss = T.sum(test_rec_loss, axis=range(1, ndim), dtype='float32')
        test_rec_penalty = (1 / C_rec) * T.mean(test_rec_loss)
    else:
        test_reconstruction = lasagne.layers.get_output(final_layer, inputs=inputs, deterministic=True)
        test_rec_penalty = T.cast(0, dtype='float32')

    test_obj = T.cast(floatX(0.5) * (l2_penalty + test_rec_penalty) + nnet.Rvar + test_loss, dtype='float32')
    nnet.forward = theano.function([inputs, targets],
...
## Code snippet added for Deep Multi-sphere SVDD 
def compile_update_msvdd(nnet, inputs, targets):
    """
    create a Deep SVDD loss for network given in argument
    """
    ...
    if nnet.R_init > 0:
        ...
        nnet.Rvar = shared(floatX(np.ones((n_cluster,1))), name="R")  # initialization with R=1
    ...
    # initialize c
    rep_dim = feature_layer.num_units
    nnet.cvar = shared(floatX(np.random.randn(n_cluster, rep_dim)), name="c")

    #compute squared distances and scores removing centers with Ri = 0
    R_ix = T.neq(nnet.Rvar,0).nonzero()[0]
    D = -2*T.dot(nnet.cvar[R_ix], rep.T) + (nnet.cvar[R_ix]**2).sum(1)[:, None] + (rep**2).sum(1).T
    dist_idx = T.argmin(D, axis=0)
    dist = D[dist_idx,T.arange(D.shape[1])]
    dist_idx = R_ix[dist_idx]
    dist = T.reshape(dist,(D.shape[1],1))
    scores = dist-nnet.Rvar[dist_idx]
    scores = T.reshape(scores,(D.shape[1],1))
    ...
    obj_ball = T.cast(floatX(0.5) * (l2_penalty + rec_penalty) + loss,
                      dtype='float32')
    updates_ball = get_updates(nnet, obj_ball, trainable_params, solver=nnet.solver)
    nnet.backprop_ball = theano.function([inputs, targets], [obj_ball, acc], updates=updates_ball,
                                         on_unused_input='warn')

    # Backpropagation (without training R) #zahra: max?min?mean?
    obj = T.cast(floatX(0.5) * (l2_penalty + rec_penalty) + T.max(nnet.Rvar) + loss,
                 dtype='float32')
    updates = get_updates(nnet, obj, trainable_params, solver=nnet.solver)
    nnet.backprop_without_R = theano.function([inputs, targets], [obj, acc], updates=updates,
                                              on_unused_input='warn')
    ...
    #compute squared distances and scores removing centers with Ri = 0
    R_ix = T.neq(nnet.Rvar,0).nonzero()[0]
    test_D = -2*T.dot(nnet.cvar[R_ix], test_rep.T) + (nnet.cvar[R_ix]**2).sum(1)[:, None] + (test_rep**2).sum(1).T
    test_dist_idx = T.argmin(test_D, axis=0)
    test_dist = test_D[test_dist_idx,T.arange(test_D.shape[1])]
    test_dist_idx = R_ix[test_dist_idx]
    test_dist = T.reshape(test_dist,(test_D.shape[1],1))
    test_scores = test_dist - nnet.Rvar[test_dist_idx]
    test_scores = T.reshape(test_scores,(test_D.shape[1],1))
    ...
    test_obj = T.cast(floatX(0.5) * (l2_penalty + test_rec_penalty) + T.max(nnet.Rvar) + test_loss, dtype='float32')
    nnet.forward = theano.function([inputs, targets],
                                   [test_obj, test_acc, test_scores, floatX(0.5) * l2_penalty,
                                    floatX(0.5) * test_rec_penalty, test_rep,
                                    test_rep_norm, test_reconstruction, test_loss, nnet.Rvar, test_dist, test_dist_idx],
                                   on_unused_input='warn')
...
def create_autoencoder(nnet):
    ...
    scores = T.sum(loss, axis=range(1, ndim), dtype='float32')
    loss = T.mean(scores)

    # Regularization
    if Cfg.ae_weight_decay:
        l2_penalty = (floatX(0.5) / C) * regularize_network_params(final_layer, l2).astype('float32')
    else:
        l2_penalty = T.cast(0, dtype='float32')

    train_obj = loss + l2_penalty
    #print(loss.type,l2_penalty.type,train_obj.type)
    ...
    rep = lasagne.layers.get_output(nnet.feature_layer, inputs=inputs, deterministic=True)
    ...
    test_scores = T.sum(test_loss, axis=range(1, ndim), dtype='float32')
    test_loss = T.mean(test_scores)

    nnet.ae_forward = theano.function([inputs],
                                      [test_loss, l2_penalty, test_scores, test_prediction, rep])