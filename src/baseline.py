'''
 ...
 Summary of the changes:
     1. Our new method, DMSVDD (msvdd for short) is added
     2. Mobifall dataset is added
     3. Normal and anomaly classes can be given as input label vectors
'''
...
from utils.log import log_exp_config, log_NeuralNet, log_AD_results, log_AD_info
...
parser.add_argument("--loss",
                    help="loss function",
                    type=str, choices=["ce", "svdd", "autoencoder", "msvdd"])
...
parser.add_argument("--momentum",
                    ...
                    type=np.float32, default=np.float32(0.9))
...
parser.add_argument("--ae_C",
                    ...
                    type=np.float32, default=np.float32(1e3))
...
parser.add_argument("--nu",
                    ...
                    type=np.float32, default=np.float32(0.1))
...
parser.add_argument("--n_cluster",
                    help="specify the number of clusters in data",
                    type=int, default=1)
...
parser.add_argument("--mobiFall_val_frac",
                    help="specify the fraction the validation set of the initial training data should be",
                    type=np.float32, default=np.float32(1./6))
parser.add_argument("--mobiFall_bias",
                    help="specify if bias terms are used in mobiFall network",
                    type=int, default=0)
parser.add_argument("--mobiFall_rep_dim",
                    help="specify the dimensionality of the last layer",
                    type=int, default=10)
...
#        theano.sandbox.cuda.use(args.device)
    ...
    Cfg.msvdd_loss = (args.loss == 'msvdd')
    ...
    Cfg.n_cluster = int(args.n_cluster)
    ...
    if Cfg.pretrain:
        nnet.pretrain(solver="adam", lr=np.float32(0.0001), n_epochs=150)
    ...
    if Cfg.ad_experiment and Cfg.plot_examples:
        n_img = 32
