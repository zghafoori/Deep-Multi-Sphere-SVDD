...
def extract_norm_and_out(X, y, normal, outlier):
    ...
    return X_normal, X_outlier, y_normal, y_outlier, idx_normal, idx_outlier
...
def learn_dictionary(X, n_filters, filter_size, n_sample=1000,
    ...
    # extract patches
    if isinstance(filter_size, (list,)): #len(filter_size) == 2:
    ...
    if isinstance(filter_size, (list,)): #len(filter_size) == 2:
        W = W.reshape(n_filters, n_channels, filter_size[0], filter_size[1])
    else:
...
def learn_dictionary_new(X, n_filters, filter_shape, n_sample=1000,
                     n_sample_patches=0, **kwargs):
    """
    learn a dictionary of n_filters atoms from n_sample images from X
    """
    ...
    dico = MiniBatchDictionaryLearning(n_components=n_filters, alpha=1, n_iter=1000, batch_size=10, shuffle=True,
                                       verbose=True, **kwargs)
    W = dico.fit(patches).components_
    W = W.reshape(n_filters, n_channels, filter_shape[0], filter_shape[1])
