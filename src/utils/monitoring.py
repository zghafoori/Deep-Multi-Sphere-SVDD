...
def performance(nnet, which_set, epoch=None, print_=False):
    ...
    dists = np.empty(n, dtype=floatX)
    dists_idx = np.empty(n, dtype=floatX)
    ...
    for batch in nnet.data.get_epoch(which_set):
        ...
        if Cfg.softmax_loss:
            ...
            err, acc, b_scores, l2, b_rec, b_rep, b_rep_norm, _, b_loss, R, b_dists, b_dists_idx = nnet.forward(inputs, targets)
            scores[start_idx:stop_idx] = b_scores.flatten()
            rep[start_idx:stop_idx, :] = b_rep
            rep_norm[start_idx:stop_idx] = b_rep_norm
            reconstruction_penalty += b_rec
            dists[start_idx:stop_idx] = b_dists.flatten()
            dists_idx[start_idx:stop_idx] = b_dists_idx.flatten()
        else:
    ...
    if Cfg.msvdd_loss:
        nnet.save_diagnostics(which_set, epoch, scores, rep_norm, rep, emp_loss, reconstruction_penalty,dists,dists_idx)
    else:
...
def ae_performance(nnet, which_set, epoch=None):
    ...
    for batch in nnet.data.get_epoch(which_set):
        ...
        err, l2, b_scores, _, b_rep = nnet.ae_forward(inputs)
